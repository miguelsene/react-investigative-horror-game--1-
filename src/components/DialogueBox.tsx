...
export const DialogueBox: React.FC<DialogueBoxProps> = ({ node, onSelectOption, onNext, onClose }) => {
  ...
  return (
    <>
      ...
      <div className="fixed inset-x-0 bottom-0 z-50 flex justify-center px-3 pb-6 sm:pb-8 pointer-events-none">
        <div onClick={skipOrAdvance} className="pointer-events-auto w-full max-w-4xl dlg-box cursor-pointer fade-up">
          <div className="flex gap-4 sm:gap-5 items-start">
            {/* Portrait */}
            <div className={`relative w-20 h-20 sm:w-24 sm:h-24 shrink-0 rounded-sm border ${portrait.border} bg-black overflow-hidden ${!done ? 'talking' : ''}`}>
              <img
                src={portrait.img}
                alt={portrait.label}
            ...
            <div className="flex-1 min-w-0">
              ...
              <p className={`font-serif-jp text-[15px] sm:text-lg leading-relaxed text-neutral-100 min-h-[3.2rem] ${isGlitch ? 'glitch-text' : ''} ${voice === 'thought' ? 'italic text-neutral-300' : ''}`}>
                {displayText}
                ...
                <div className="mt-4 flex flex-col gap-1.5 fade-up" onClick={(e) => e.stopPropagation()}>
                  {node.options.map((opt, idx) => (
                    <button
                      ...
                      className="group text-left font-serif-jp text-sm text-neutral-300 hover:text-white transition-colors flex items-baseline gap-3 py-1"
                    >
