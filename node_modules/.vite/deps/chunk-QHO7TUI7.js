...
} from "./chunk-IA72QBLE.js";
import {
  Scene
} from "./chunk-DWQ3YTU3.js";
import {
  UniqueIdGenerator
...
var RuntimeAnimation = class {
  ...
  constructor(target, animation, scene, host) {
    ...
    this._playbackFrom = 0;
    this._playbackTo = 0;
    this._playbackFrames = 0;
    this._playbackProgress = 0;
    this._playbackSyncRoot = null;
    this._playbackJumped = false;
    this._playbackSyncMasterPlayed = 0;
    this._playbackSyncMasterFrame = 0;
...
var AnimationGroup = class _AnimationGroup {
  ...
  constructor(name, scene = null, weight = -1, playOrder = 0) {
    ...
    this._virtualFrameStart = 0;
    this._virtualFrameStartTime = 0;
    this._virtualFrameRate = 60;
    this._virtualFrameDirection = 1;
    this._virtualFrameInitial = 0;
    this._virtualFrameEnd = 0;
    this._virtualSamplingObserver = null;
    this._usesVirtualSampling = false;
    this._retainedCurrentFrame = 0;
   ...
   * @returns the current animation group
   */
  startWithVirtualTimeline(loop = false, speedRatio = 1, from, to, isAdditive) {
    return this._start(loop, speedRatio, from, to, isAdditive, true);
  }
  _start(loop, speedRatio, from, to, isAdditive, useVirtualTimeline) {
    ...
    const effectiveSamplingTo = useVirtualTimeline ? Number.isFinite(effectiveTo) ? this._mapVirtualFrame(effectiveTo) : effectiveTo < effectiveFrom ? 0 : this._to : effectiveTo;
    ...
    for (let index = 0; index < this._targetedAnimations.length; index++) {
      const targetedAnimation = this._targetedAnimations[index];
      const animatable = this._scene.beginDirectAnimation(targetedAnimation.target, [targetedAnimation.animation], samplingFrom, samplingTo, this._usesVirtualSampling || loop, speedRatio, void 0, void 0, isAdditive !== void 0 ? isAdditive : this._isAdditive);
   ...
   */
  isValidVirtualStopFrame(frame) {
    return this._virtualFrameDirection > 0 ? frame >= this._virtualFrameInitial && frame < this._virtualFrameEnd : frame <= this._virtualFrameInitial && frame > this._virtualFrameEnd;
  }
  /**
   * Snaps both retained playheads to an exact virtual timeline position.
   * @param frame virtual frame to retain
   */
  setVirtualCurrentFrame(frame) {
  ...
  _updateVirtualFrame() {
    ...
    const elapsedSeconds = (this._scene._animationTime - this._virtualFrameStartTime) / 1e3;
  ...
  _sampleVirtualTimeline() {
    this._updateVirtualFrame();
    const reachedEnd = !this._loopAnimation && Number.isFinite(this._virtualFrameEnd) && (this._virtualFrameDirection > 0 ? this._virtualFrame >= this._virtualFrameEnd : this._virtualFrame <= this._virtualFrameEnd);
    if (reachedEnd) {
...
//# sourceMappingURL=chunk-QHO7TUI7.js.map
