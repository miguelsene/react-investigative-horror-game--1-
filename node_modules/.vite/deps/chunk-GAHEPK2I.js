import {
  Texture
} from "./chunk-JZ2Q4TKW.js";
import {
  EffectWrapper
} from "./chunk-DHC2TZIH.js";
import {
  PostProcessManager,
  RenderingManager,
  SmartArray
...
var ObjectRenderer = class _ObjectRenderer {
  ...
  render(passIndex = 0, skipOnAfterRenderObservable = false) {
    ...
    if (!fastPath) {
      ...
      this._renderingManager.render(this.customRenderFunction, currentRenderList, this.renderParticles, this.renderSprites, this.renderDepthOnlyMeshes, this.renderOpaqueMeshes, this.renderAlphaTestMeshes, this.renderTransparentMeshes, this.customRenderTransparentSubMeshes, this.spriteManagerList);
  ...
  _checkReadiness() {
    ...
    for (let passIndex = 0; passIndex < numPasses && returnValue; passIndex++) {
      ...
      for (let i = 0; i < currentRenderListLength && returnValue; ++i) {
        ...
        } else if (mesh.hasInstances && mesh.edgesShareWithInstances) {
          useInstances = true;
        }
...
var RenderTargetTexture = class _RenderTargetTexture extends Texture {
  ...
  isReadyForRendering() {
    if (!this._dumpToolsLoading) {
      ...
      import("./dumpTools.pure-Q43S32OG.js").then((module) => this._dumpTools = module);
    }
  ...
  return _a = class PostProcess {
    ...
    _gatherImports(useWebGPU = false, list) {
      if (useWebGPU && this._webGPUReady) {
        list.push(Promise.all([import("./postprocess.vertex-H3AHAKM5.js")]));
      } else {
        list.push(Promise.all([import("./postprocess.vertex-RUNGRP76.js")]));
      }
    }
    /**
...
var ThinPassPostProcess = class _ThinPassPostProcess extends EffectWrapper {
  _gatherImports(useWebGPU, list) {
    if (useWebGPU) {
      ...
      list.push(Promise.all([import("./pass.fragment-VVCESDXU.js")]));
    } else {
      list.push(Promise.all([import("./pass.fragment-D63ZXRJW.js")]));
    }
...
var ThinPassCubePostProcess = class _ThinPassCubePostProcess extends EffectWrapper {
  _gatherImports(useWebGPU, list) {
    if (useWebGPU) {
      this._webGPUReady = true;
      list.push(Promise.all([import("./passCube.fragment-TKFY62NC.js")]));
    } else {
      list.push(Promise.all([import("./passCube.fragment-ACWOC25D.js")]));
    }
    super._gatherImports(useWebGPU, list);
  }
  /**
...
async function ReadPixelsUsingRTT(texture, width, height, face, lod, slice = 0) {
  ...
  if (!engine.isWebGPU) {
    if (is3DTexture) {
      await import("./lod3D.fragment-2UNLNG6G.js");
    } else if (texture.isCube) {
      await import("./lodCube.fragment-IA6Q6UVG.js");
    } else {
      await import("./lod.fragment-27OOIBQT.js");
    }
  } else {
    if (is3DTexture) {
      await import("./lod3D.fragment-CGIKWWCU.js");
    } else if (texture.isCube) {
      await import("./lodCube.fragment-UXTCZGZB.js");
    } else {
      await import("./lod.fragment-PZHHGAMS.js");
    }
  }
  let lodPostProcess;
  if (is3DTexture) {
...
//# sourceMappingURL=chunk-GAHEPK2I.js.map
