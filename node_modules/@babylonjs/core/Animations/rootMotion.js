 ...
 */
export var RootMotionSource;
(function (RootMotionSource) {
    /**
     * No motion was found: the clip neither moves its root nor walks its contact nodes.
     */
    RootMotionSource[RootMotionSource["None"] = 0] = "None";
    /**
     ...
     */
    RootMotionSource[RootMotionSource["Root"] = 1] = "Root";
    /**
     ...
     */
    RootMotionSource[RootMotionSource["FootContact"] = 2] = "FootContact";
})(RootMotionSource || (RootMotionSource = {}));
/**
 ...
 */
function ClockChannel(channels, fromFrame, toFrame) {
    let best = null;
 ...
 */
function ClockBeats(candidate, other) {
...
const SceneRootMotions = new WeakMap();
/**
...
export class RootMotionClip {
     ...
     */
    constructor(animationGroup, options = {}) {
        /**
         * @internal
        ...
        this._lateralDrift = Vector3.Zero();
        this._channels = new Map();
        this._source = 0 /* RootMotionSource.None */;
        this._rootNode = null;
        this._contactNodes = [];
        this._travelDirection = new Vector3(0, 0, 1);
        this._cycleOffset = Vector3.Zero();
        this._cycleRotation = 0;
        this._turns = false;
        this._duration = 0;
        ...
        this._restoredPosition = null;
        this._clockIndex = -1;
        this._sampleBlend = 0;
        this._lastOffset = Vector3.Zero();
        this._rangeStart = Vector3.Zero();
        this._rangeCycle = Vector3.Zero();
        this._cyclesOffset = Vector3.Zero();
        this._planarStep = Vector3.Zero();
        this._yawQuaternion = new Quaternion();
        this._yawMatrix = new Matrix();
        ...
        if (animationGroup.isAdditive) {
            throw new Error(`RootMotionClip: animation group "${animationGroup.name}" is additive. Build the clip from the group it was made additive from, then make the clip's in-place group additive.`);
        }
        ...
        const refuseAnimatedReceiver = (node) => {
            if (targetedAnimations.some((targetedAnimation) => targetedAnimation.target === node && /^(position|rotationQuaternion|rotation)(\.|$)/.test(targetedAnimation.animation.targetProperty))) {
                throw new Error(`RootMotionClip: animation group "${animationGroup.name}" animates the character node "${node.name}", which is to receive the motion. Parent the character under a node the group does not animate, or pass such an ancestor as characterNode.`);
            }
        };
        ...
        this._turns = !!yaw;
        if (source === 1 /* RootMotionSource.Root */ && !rootChannels?.position) {
            throw new Error(`RootMotionClip: the root node "${root.name}" has no position animation to take the travel from. Omit the source to deduce it from the contact nodes, or pass another rootNode.`);
        }
        let translation = null;
        if ((source === undefined || source === 1 /* RootMotionSource.Root */) && root && rootChannels?.position) {
            translation = this._measureRootTravel(yaw, minimumTravel, source === 1 /* RootMotionSource.Root */);
            if (translation) {
                this._source = 1 /* RootMotionSource.Root */;
                this._removesRootTravel = true;
            }
        }
        if (!translation && (source === undefined || source === 2 /* RootMotionSource.FootContact */)) {
            translation = this._measureContactTravel(yaw, options.contactNodes, minimumTravel, source === 2 /* RootMotionSource.FootContact */);
            if (translation) {
                this._source = 2 /* RootMotionSource.FootContact */;
            }
        }
        if (!translation && yaw) {
     ...
     * @returns the turn in radians
     */
    _motionBetween(from, to, lastProgress, progress, result) {
        const range = to - from;
        if (range === 0 || (this._source === 0 /* RootMotionSource.None */ && !this._turns)) {
            result.setAll(0);
            return 0;
        }
     ...
     * @returns the turn
     */
    _motionInRange(frame, startYaw, startOffset, result) {
     ...
     */
    _cyclePower(cycles, step, stepYaw, result) {
    ...
    _measureRootTravel(yaw, minimumTravel, forced) {
        const root = this._rootNode;
        const samples = this._sampleCount;
        const positions = [];
        for (let i = 0; i <= samples; i++) {
     ...
     * @returns the result vector
     */
    _rootTravelAt(position, start, yaw, turning, result) {
        if (turning) {
    ...
    _measureContactTravel(yaw, explicitContacts, minimumTravel, forced) {
        const samples = this._sampleCount;
        const up = this._upAxis;
        let contacts;
        if (explicitContacts) {
            contacts = explicitContacts.map((node) => this._resolveNode(node)).filter((node) => !!node);
        }
        else {
            const base = this._rootNode ?? this._characterNode;
            contacts = base.getDescendants(false, IsJoint).filter((node) => !node.getChildren(IsJoint, true).length);
        }
        if (!contacts.length) {
            return null;
        }
     ...
     */
    _buildInPlaceGroup(name, cloneAnimations) {
        ...
        const inPlaceAt = (frame, position, rotation) => {
        ...
        const inPlaceKeys = (sourceKeys, kind) => {
            const keys = [];
            ...
            for (const key of sourceKeys) {
                ...
                const tangent = (original, direction) => {
                    if (!original) {
                        return original;
                    }
    ...
    _measureHeight(base) {
        ...
        for (const node of [base, ...base.getDescendants(false, IsJoint)]) {
     ...
     */
    _snapTravelDirection(defaultAngle) {
    ...
    _rotateAboutUp(vector, angle, result) {
        if (angle === 0) {
     ...
     */
    _twist(rotation, referenceConjugate) {
     ...
     */
    _matrixToCharacter(node, frame, result) {
    ...
    _localMatrix(node, frame, result) {
        const channels = this._channels.get(node);
        const scaling = channels?.scaling ? channels.scaling.evaluate(frame) : node.scaling;
        const position = channels?.position ? channels.position.evaluate(frame) : node.position;
        let rotation;
        if (channels?.rotationQuaternion) {
...
export class RootMotionController {
     ...
     */
    constructor(characterNode, clips = []) {
        /**
         ...
         */
        this.onRootMotionObservable = new Observable();
        this._clips = [];
        this._upAxis = null;
    ...
    addClip(clip) {
        ...
        if (clip.characterNode !== this._characterNode) {
            throw new Error(`RootMotionController: clip "${clip.animationGroup.name}" moves "${clip.characterNode.name}", not "${this._characterNode.name}". Pass the controller's node as the clip's characterNode.`);
        }
     ...
     */
    _accumulate(clip, runtime, lastProgress, progress) {
        if ((clip.source === 0 /* RootMotionSource.None */ && !clip.extractsRotation) || !clip._writers) {
            return false;
        }
     ...
     */
    _effectiveWeight(writers, runtime) {
 ...
 * @returns the clone
 */
function CloneAnimation(source, cloneKeys = false) {
