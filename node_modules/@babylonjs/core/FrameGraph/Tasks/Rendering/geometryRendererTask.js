...
export class FrameGraphGeometryRendererTask extends FrameGraphObjectRendererTask {
     ...
     */
    get objectIdProvider() {
        return this._objectIdProvider;
    }
    set objectIdProvider(value) {
     ...
     */
    get meshBlendTagProvider() {
        return this._meshBlendTagProvider;
    }
    set meshBlendTagProvider(value) {
    ...
    _checkParameters() {
        ...
        const objectIdDescription = this.textureDescriptions.find((description) => description.type === 15);
        if (objectIdDescription) {
            ...
            const isSupportedFormat = objectIdDescription.textureFormat === 5 || objectIdDescription.textureFormat === 6;
            if (objectIdDescription.textureType !== 0 || !isSupportedFormat) {
        ...
        const meshBlendTagIndex = this.textureDescriptions.findIndex((description) => description.type === 16);
        if (meshBlendTagIndex !== -1) {
            ...
            if (description.textureType !== 0 || description.textureFormat !== 8) {
    ...
    _buildClearAttachmentsLayout() {
        ...
        const targetTextureCount = this._getTargetTextureCount();
        const allAttachmentsLayout = new Array(targetTextureCount).fill(true);
        for (let i = 0; i < this.textureDescriptions.length; i++) {
            const description = this.textureDescriptions[i];
            const index = MaterialHelperGeometryRendering.GeometryTextureDescriptions.findIndex((f) => f.type === description.type);
            const geometryDescription = MaterialHelperGeometryRendering.GeometryTextureDescriptions[index];
    ...
    _registerForRenderPassId(renderPassId) {
        ...
        for (let i = 0; i < this.textureDescriptions.length; i++) {
            ...
            if (description.type === 15) {
                configuration.objectIdIsRedFormat = description.textureFormat === 6;
            }
            else if (description.type === 16) {
