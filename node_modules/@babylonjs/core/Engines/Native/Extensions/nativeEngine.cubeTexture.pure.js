...
function ApplySphericalPolynomialFromNative(texture, sphericalPolynomial) {
...
function SyncCubeTextureSizeFromNative(nativeEngine, texture, fallbackSize) {
...
function NotifyCubeTextureLoaded(texture, onLoad, loadData) {
...
export function RegisterNativeEngineCubeTexture() {
    ...
    ThinNativeEngine.prototype.createCubeTexture = function (rootUrl, scene, files, noMipmap, onLoad = null, onError = null, format, forcedExtension = null, createPolynomials = false, lodScale = 0, lodOffset = 0, fallback = null, loaderOptions, useSRGBBuffer = false, buffer = null) {
        ...
        if (extension === ".env") {
            ...
            const loadContainerBufferAsync = async (data) => {
                const view = data instanceof Uint8Array ? data : new Uint8Array(data.buffer, data.byteOffset, data.byteLength);
                await new Promise((resolve, reject) => {
                    this._engine.loadCubeTexture(texture._hardwareTexture.underlyingResource, [view], !noMipmap, false, texture._useSRGBBuffer, (sphericalPolynomial) => {
    ...
    ThinNativeEngine.prototype.createPrefilteredCubeTexture = function (rootUrl, scene, lodScale, lodOffset, onLoad = null, onError = null, format, forcedExtension = null, createPolynomials = true, buffer = null) {
        const texture = new InternalTexture(this, 9 /* InternalTextureSource.CubePrefiltered */);
        if (!createPolynomials) {
            texture._sphericalPolynomial = new SphericalPolynomial();
        }
        return this.createCubeTexture(rootUrl, scene, null, false, () => {
            if (onLoad) {
                onLoad(texture);
            }
        }, onError, format, forcedExtension, createPolynomials, lodScale, lodOffset, texture, undefined, false, buffer);
    };
}
