...
export class FlowGraph {
    ...
    _attachEventObserver() {
        ...
        this._eventObserver = this._sceneEventCoordinator.onEventTriggeredObservable.add((event, eventState) => {
            ...
            const propagationStops = event.type === "MeshPick" /* FlowGraphEventType.MeshPick */ || event.type === "PointerOver" /* FlowGraphEventType.PointerOver */ || event.type === "PointerOut" /* FlowGraphEventType.PointerOut */
                ? []
                : undefined;
    ...
    _getContextualOrder(type, context) {
        ...
        if (type === "MeshPick" /* FlowGraphEventType.MeshPick */ || type === "PointerOver" /* FlowGraphEventType.PointerOver */ || type === "PointerOut" /* FlowGraphEventType.PointerOut */) {
            const meshPickOrder = [];
            for (const block1 of order) {
                const mesh1 = type === "MeshPick" /* FlowGraphEventType.MeshPick */
                    ? block1.asset.getValue(context)
                    : block1._getReferencedMesh(context);
                let i = 0;
                for (; i < order.length; i++) {
                    const block2 = order[i];
                    const mesh2 = type === "MeshPick" /* FlowGraphEventType.MeshPick */
                        ? block2.asset.getValue(context)
                        : block2._getReferencedMesh(context);
