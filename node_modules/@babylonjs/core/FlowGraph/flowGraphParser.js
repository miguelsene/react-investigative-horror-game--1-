...
import { _GetDefaultEventDataParseCount, defaultValueParseFunction, needsPathConverter } from "./serialization.js";
import { getRichTypeByFlowGraphType, RichType } from "./flowGraphRichTypes.pure.js";

import { WebRequest } from "../Misc/webRequest.js";
function ParseCustomEventData(serializedEventData, valueParseFunction, assetsContainer, scene) {
    const eventData = Object.create(null);
    if (Array.isArray(serializedEventData)) {
        for (const serializedEntry of serializedEventData) {
            if (!serializedEntry?.eventData) {
                continue;
            }
            const entry = { type: getRichTypeByFlowGraphType(serializedEntry.type) };
    ...
    if (serializedEventData === null || typeof serializedEventData !== "object") {
        return eventData;
    }
    for (const eventKey of Object.keys(serializedEventData)) {
        const serializedEntry = serializedEventData[eventKey];
        if (serializedEntry === null || typeof serializedEntry !== "object") {
            continue;
        }
        const entry = { type: serializedEntry.type };
        if (serializedEntry.value !== undefined) {
            entry.value = serializedEntry.value?.getClassName?.() ? serializedEntry.value : valueParseFunction("value", serializedEntry, assetsContainer, scene);
        }
...
export function ParseFlowGraphBlockWithClassType(serializationObject, parseOptions, classType) {
    ...
    if (serializationObject.config) {
        for (const key in serializationObject.config) {
            ...
            const isCustomEventData = key === "eventData" &&
                (serializationObject.className === "FlowGraphSendCustomEventBlock" /* FlowGraphBlockNames.SendCustomEvent */ || serializationObject.className === "FlowGraphReceiveCustomEventBlock" /* FlowGraphBlockNames.ReceiveCustomEvent */);
