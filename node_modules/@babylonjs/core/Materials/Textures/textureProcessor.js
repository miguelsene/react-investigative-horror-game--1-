 ...
 */
function _CreateProcessorTexture(name, defines, outputSize, scene, outputColorSpace = TextureColorSpace.Linear, outputOptions) {
    const options = {
        type: outputOptions?.textureType ?? 0,
        format: 5,
        samplingMode: outputOptions?.samplingMode ?? 2,
 ...
 */
export async function MultiplyTexturesAsync(name, a, b, scene, outputColorSpace, outputChannelMask, outputOptions) {
 ...
 * @returns An operand whose `texture` holds the GPU result, or whose `factor` holds the CPU-folded constant
 */
export async function MaxTexturesAsync(name, a, b, scene, outputColorSpace, outputChannelMask, outputOptions) {
    if (!a.texture && !b.texture) {
 ...
 * @returns An operand whose `texture` holds the GPU result, or whose `factor` holds the CPU-folded constant
 */
export async function LerpTexturesAsync(name, a, b, t, scene, outputColorSpace, outputChannelMask, outputOptions) {
 ...
 * @returns An operand whose `texture` holds the GPU result, or whose `factor` holds the CPU-folded constant
 */
export async function InvertTextureAsync(name, input, scene, channels = ChannelMask.RGBA, outputColorSpace, outputChannelMask, outputOptions) {
    if (!input.texture) {
 ...
 * @returns An operand whose `texture` holds the GPU result, or whose `factor` holds the CPU-folded constant
 */
export async function ExtractMaxChannelAsync(name, input, scene, includeAlpha = false, outputColorSpace, outputChannelMask, outputOptions) {
    if (!input.texture) {
        const c = _EvalConstant(input);
 ...
 * @returns An operand whose `texture` holds the GPU result, or whose `factor` holds the CPU-folded constant
 */
export async function ExtractChannelAsync(name, input, channel, scene, outputColorSpace, outputChannelMask, outputOptions) {
    if (!input.texture) {
