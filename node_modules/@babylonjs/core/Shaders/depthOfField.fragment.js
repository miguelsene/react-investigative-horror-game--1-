...
uniform float aperture;uniform float darken;uniform float edge_blur;uniform bool highlights;uniform float blur_lod;uniform float near;uniform float far;varying vec2 vUV;
...
vec4 sharpColor=texture2D(textureSampler,distorted_coords);float coc=abs(aperture*(screen_distance*(inverse_focal_length-1.0/distance)-1.0));if (dof_enabled==false || coc<0.07) { coc=0.0; }
float edge_blur_amount=0.0;if (edge_blur>0.0) {edge_blur_amount=clamp((radius*2.0-1.0+0.15*edge_blur)*1.5,0.0,1.0)*1.3;}
float blur_amount=max(edge_blur_amount,coc);if (blur_amount==0.0) {gl_FragColor=sharpColor;}
else {gl_FragColor=getBlurColor(blur_amount*1.7);if (highlights) {gl_FragColor.rgb+=clamp(coc,0.0,1.0)*TEXTUREFUNC(highlightsSampler,distorted_coords,0.0).rgb;}
if (blur_noise) {vec2 noise=rand(distorted_coords)*0.01*blur_amount;vec2 blurred_coord=vec2(distorted_coords.x+noise.x,distorted_coords.y+noise.y);gl_FragColor=0.04*TEXTUREFUNC(textureSampler,blurred_coord,blur_lod)+0.96*gl_FragColor;}}
